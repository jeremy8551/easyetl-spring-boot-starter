...
import icu.etl.springboot.starter.script.SpringEasyEnvironment;
...
public class SpringEasyConfiguration {
    ...
    public UniversalScriptEngine getScriptEngine(EasyContext context, ApplicationContext springContext) {
        UniversalScriptEngine engine = this.getScriptEngineFactory(context).getScriptEngine();
        engine.setBindings(new SpringEasyEnvironment(springContext), UniversalScriptContext.ENVIRONMENT_SCOPE);
        return engine;
    }
